...
export function newsEncode(html_string: string,ads=false) {
    const $ = load(html_string,null,false);
    
    let html = $.html();

    if(ads) {
        html = html.replace(/\[IKLAN_IKLAN\]\<\/p\>/,"</p><div data-portalnesia-action=\"ads\" data-ads=\"300\"></div>");
        html = html.replace(/\[IKLAN_IKLAN_IKLAN\]\<\/p\>/,"</p><div data-portalnesia-action=\"ads\" data-ads=\"468\"></div>");
    } else {
        html = html.replace(/\[IKLAN_IKLAN\]\<\/p\>/,"</p>");
        html = html.replace(/\[IKLAN_IKLAN_IKLAN\]\<\/p\>/,"</p>");
    }
    return html;
}