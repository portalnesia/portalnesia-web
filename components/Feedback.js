...
import homeStyle from 'portal/styles/Home.module.css'
...
export const getSysInfo=(string)=>{
    let sysInfo={};
    const browser=deviceDetect();
    if(typeof browser.ua !== 'undefined') delete browser.ua
    sysInfo.origin = window.location.href;
    if(string===true) sysInfo.browserDetail = JSON.stringify(browser);
    else sysInfo.browserDetail = browser
...
class Feedback extends React.Component {
    constructor(props){
        ...
        this.screenshotPrev=React.createRef()
    ...
    getSysInfo() {
        ...
        const accInfo=this.props.user===null ? {} : {Accounts:this.props.user}
    ...
    send() {
        ...
        if(this.state.loading && this.state.shotOpen) {
            this.props.enqueueSnackbar(this.props.loadingTip,{
                ...
                action:(key)=>(
                    <IconButton
                        className={homeStyle.snackbarIcon}
        ...
        if(this.props.required && text.length === 0) {
            this.setState({
                textError: this.props.requiredTip,
            });
    ...
    render(){
        ...
        return(
            ...
            <div id="googleFeedback" style={{height: `${state.docHeight}px`}} onMouseMove={this.handleMouseMove.bind(this)}
                    ...
                    !isMobile ?
                                    ...
                                    <div id="feedbackDialog" className="dialog" data-html2canvas-ignore="true" style={{left: '50%', top: '50%',maxHeight:`${state.winHeight-40}px`,marginTop:`-${(state.winHeight-40)/2}px`}}>
                                        <div {...(state.showInformation ? {style:{display:'flex'}} : {})}>
                                            {state.showInformation && (
                                                <span disabled={props.disabled} className={`close btn${props.disabled ? ' disabled':''}`} style={{cursor:'pointer',background: props.theme || '#3986FF',width:56,height:56,alignItems:'center'}} onClick={this.showInformation.bind(this)}>
                                        ...
                                        <div className="feedback-area">
                                                        ...
                                                        if(typeof this.sysInfo?.[dt]==='object' && dt!== 'mimeType') {
                                                            ...
                                                            return(
                                                                <p key={i} style={{marginBottom:'1em'}}>
                                                                    ...
                                                                    <span style={{color:'#000000',marginLeft:10}} key={`${i}-1`}>{this.sysInfo?.[dt].toString()}</span>
                                                                </p>
                                                            )
                                                        }
                                                    })}
                                            ...
                                            <div style={{overflowY:'auto',maxHeight:`${state.winHeight-40-112}px`}}>
                                                ...
                                                <div className="shot-switch clearfix">
                                                    <div className={`checkbox${props.disabled ? ' disabled':''}`} onClick={this.checkboxHandle.bind(this)} disabled={props.disabled}>
                                            ...
                                            <div className="actions">
                                                <div disabled={props.disabled} className={`flatbutton cancel${props.disabled ? ' disabled':''}`} style={{color: '#757575'}} onClick={this.cancel.bind(this)}>{props.cancelLabel}</div>
                                                <div disabled={props.disabled} className={`flatbutton confirm${props.disabled ? ' disabled':''}`}
                        ...
                        <div className="mobile-feedBack-window" data-html2canvas-ignore="true" style={{height: `${state.winHeight}px`}}>
                            <div className="header">
                                <div className="left">
                                    {state.showInformation ? (
                                        <span disabled={props.disabled} className={`close btn${props.disabled ? ' disabled':''}`} onClick={this.showInformation.bind(this)}>
                                            <ArrowBack className='svgIcon' />
                                        </span>
                                    ) : (
                                        <span disabled={props.disabled} className={`close btn${props.disabled ? ' disabled':''}`} onClick={this.cancel.bind(this)}>
                                ...
                                {state.showInformation ? null : 
                                    <span disabled={props.disabled} style={{position:'relative'}} className={`send btn${props.disabled ? ' disabled':''}`} onClick={this.send.bind(this)}>
                                        {props.disabled && (
                                            <div className="loading-icon">
                                                <svg viewBox="0 0 40 40" style={{width: '40px', height: '40px', position: 'relative'}}>
                                                    <circle cx="20" cy="20" r="18.25" fill="none" strokeWidth="3.5"
                                                            strokeMiterlimit="20"
                                                            style={{stroke: '#fff', strokeLnecap: 'round'}}></circle>
                                                </svg>
                                            </div>
                                        )}
                            ...
                            <div className="main">
                                ...
                                <div className="screenshot">
                                    <div className="shot-switch clearfix">
                                        <div disabled={props.disabled} className={`checkbox${props.disabled ? ' disabled':''}`} onClick={this.checkboxHandle.bind(this)}>
                ...
                <canvas ref={this.canvas} id="feedbackCanvas" data-html2canvas-ignore="true"
                    onMouseDown={this.canvasMouseDown.bind(this)}
                ></canvas>
                <canvas ref={this.shadowCanvas} id="shadowCanvas"
                ></canvas>
...
const mapToProps=state=>({
    user:state.user===null ? null : {
        username:`@${state.user.user_login}`,
        name:state.user.user_nama,
        email:state.user.user_email
    }
})
export default connect(mapToProps)(withSnackbar(Feedback))