...
{data?.length > 5 && i!==0 && i % 5 === 0 ? (
                                <Grid key={`blog-${dt.slug}-0`} item xs={12} sm={6} md={4} lg={3}>
                                    <AdsRect />
                                </Grid>
                            ) : null}
*/

const BlogTag=({classes,err})=>{
    ...
    const getData=(pg,aaa)=>{
        ...
        if(!isReachEnd && !keyboard && !feedback) {
            setIsLoading(true);
            get(`/v1/blog/tags/${slug}?page=${pg}`,{error_notif:false})
            .then(([res])=>{
        ...
        const onScroll=()=>{
            ...
            if((scrollTop + docHeight) > (scrollHeight-150)) {
                if(!isLoading && !isReachEnd && !keyboard && !feedback) {
                    ...
                    },`/blog/tags/${slug}/${pgg ? '?page='+pgg : ''}`,{shallow:true})
                }
            }
        }
    ...
    const handleFirstPage=()=>{
        if(!isLoadingFP && firstPage>1) {
            setIsLoadingFP(true);
            get(`${process.env.API}/blog/tags/${slug}?page=${firstPage - 1}`,{error_notif:false})
            .then(([res])=>{
    ...
    return (
        <Header iklan title={`${title} Tags - Blog`} desc={`Post with ${title} tags`} keyword="blog,platform,writer" active='blog' subactive='blog_tags' canonical={`/blog/tags/${slug}`}>
            <Breadcrumbs routes={[{label:"Blog",href:"/blog"}]} title={`${title} Tags`} />
            <PaperBlock title={`${title} Tags`} whiteBg>
...
export default withStyles(BlogTag,styles)