...
type IBlogEdit = Pick<BlogDetail,'title'|'format'|'text'|'tags'|'category'|'slug'|'publish'|'image'|'link'|'block'>
...
export const getServerSideProps = wrapper<IBlogEdit>(async({redirect,session,params,resolvedUrl,fetchAPI})=>{
    ...
    try {
        const data: IBlogEdit = await fetchAPI<IBlogEdit>(`/v2/blog/dashboard/${slug}`);
        return {
            props:{
                data,
                meta:{
...
export default function EditBlogAdmin({data,meta}: IPages<IBlogEdit>) {
    ...
    useBeforeUnload(canChange,`/admin/blog/${slug}`)
    ...
    return (
        <Pages title={meta?.title} noIndex canonical={`/admin/blog/${data?.slug}`}>
            <DashboardLayout adminPage>
                <Breadcrumbs title="Edit Blog" routes={[{
                    label:"Admin",
                    link:"/admin"
                },{
                    label:"Blog",
                ...
                <Box borderBottom={theme=>`2px solid ${theme.palette.divider}`} pb={0.5} mb={5}>
                    <Typography variant='h3' component='h1'>Edit Blog</Typography>
                </Box>
