...
type IBlogEdit = Pick<CopyPartial<BlogDetail,'link'|'slug'>,'title'|'format'|'text'|'tags'|'category'|'slug'|'publish'|'image'|'link'|'block'>
...
export default function EditPagesAdmin({data,meta}: IPages<IBlogEdit>) {
    ...
    const urlLink = React.useMemo(()=>portalUrl(`/pages/${slugFormat(input.slug||"")}`),[input.slug])
    ...
    useBeforeUnload(canChange,`/admin/pages/${slug?.[0]}`)
    ...
    return (
        <Pages title={meta?.title} noIndex canonical={'slug' in data ? `/admin/pages/${data?.slug}` : '/admin/pages/new'}>
            <DashboardLayout adminPage>
                <Breadcrumbs title={'slug' in data ? "Edit Pages" : "New Pages"} routes={[{
                    label:"Admin",
                    link:"/admin"
                },{
                    label:"Pages",
                ...
                <Box borderBottom={theme=>`2px solid ${theme.palette.divider}`} pb={0.5} mb={5}>
                    <Typography variant='h3' component='h1'>{'slug' in data ? "Edit Pages" : "New Pages"}</Typography>
                </Box>
                                                    ...
                                                    <Grid item xs={12}>
                                                        <FormGroup>
                                                            <FormControlLabel control={
                                                                <Switch disabled={loading} checked={input.block} onChange={event=>handleChange('block',event.target.checked)} color="primary" />
                                                            }
                                                            label="Block" />
                                                        </FormGroup>
                                                    </Grid>
