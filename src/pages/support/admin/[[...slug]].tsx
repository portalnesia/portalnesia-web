...
export const getServerSideProps = wrapper<ISupportPage>(async({redirect,resolvedUrl,session,params,fetchAPI})=>{
    ...
    if(support_id) {
        ...
        try {
            const support:ISupportRoom = await fetchAPI<ISupportRoom>(`/v2/support/admin/${support_id}/check`);
...
function getLabel(status: string) {
    const color = status === 'close' ? 'error' : status === 'customer reply' ? 'primary' : 'default'
...
export default function SupportPage({data:{support:supportServer}}: IPages<ISupportPage>) {
    ...
    const [title,setTitle] = React.useState(()=>supportServer ? `${supportServer.subject} - Support Admin` : 'Support Admin');
    const {data:supports,size,isLoadingMore,setSize,mutate} = useSWRPagination<PaginationResponse<ISupportRoom>>(`/v2/support/admin`);
    ...
    React.useEffect(()=>{
        if(selected) setTitle(`${selected.subject} - Support Admin`);
        else {
    ...
    return (
        <Pages title={title} noIndex withoutShowTop canonical={`/support/admin${typeof slug?.[0] === 'string' ? `/${slug?.[0]}` : ''}`}>
            {ready && (
                <Box display={{xs:'block',md:'flex'}} minHeight='100%'>
                    <NavbarChat title={is432 ? false : "Admin"}>
                        {selected && (
                            <>
                                <Fade in={img!==null}>
                    ...
                    <Slide in={(selected===undefined && mdDown) || !mdDown} direction='right'>
                        <SidebarChat title={is432 ? false : "Admin"} active={isActive} navbar={navbar} linkProps={{shallow:true}}>
                    ...
                    <Table>
                        <TableBody>
                            ...
                            <TableRow>
                                ...
                                <TableCell>{selected?.user ? <Link href={`/user/${selected.user.username}`}><Span sx={{color:'customColor.link'}}>{selected?.ticket?.name}</Span></Link> : selected?.ticket?.name}</TableCell>
                            </TableRow>
                            <TableRow>
...
function ChatComp({selected,img,setImg,mutateRoom}: ChatCompProps) {
    const {data,error,size,setSize,mutate,isLoadingMore,isLoading} = useSWRPagination<PaginationResponse<IMessages>>(`/v2/support/admin/${selected.id_number}`)
    ...
    const handleSend = React.useCallback(submitForm(async()=>{
        try {
            ...
            await post<IMessages>(`/v2/support/admin/${selected.id}`,form,{
                headers:{
            ...
            if(selected.status !== 'answered') mutateRoom()
            setTimeout(()=>{
